...
function Invoke-Mamba() {
    ...
    if ($Args.Count -eq 0) {
        # No args, just call the underlying conda executable.
        & $Env:MAMBA_EXE $Env:_CE_M $Env:_CE_CONDA;
    }
    else {
        $Command = $Args[0];
        if ($Args.Count -ge 2) {
        ...
        switch ($Command) {
            ...
            default {
                ...
                # differently in the shell wrapper, pass it through
                # verbatim.
                & $Env:MAMBA_EXE $Env:_CE_M $Env:_CE_CONDA $Command @OtherArgs;
            }
        }
    }
}
...
function Expand-CondaSubcommands() {
    ...
    Invoke-Mamba commands | Where-Object { $_ -like "$Filter*" } | Write-Output;
...
New-Alias mamba Invoke-Mamba -Force
